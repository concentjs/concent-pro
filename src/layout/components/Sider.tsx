...
function setup(ctx: CtxDe) {
  ...
  const setActiveRoutePath = (path: string) => {
    const { search } = extractPathAndSearch(window.location.href);
...
export function SiderMenus(props: ISiderMenusProps) {
  ...
  const { showingMenus } = getMenuData();

  return (
    <div className={`${styles.siderMenusWrapOfHub} smallScBar`} style={{ width: gcu.siderStyle.width }}>
      <Menu
        theme={theme}
        mode={mode}
...
// 此处强写了 backgroundColor 为主题色，防止light模式下，鼠标悬浮在按钮上导致按钮变白，从而用户看不到按钮了
// const stBtn = { width: '100%', border: 'none', backgroundColor: 'var(--theme-color)' };
const stBtn = {
  width: '100%', border: 'none', boxShadow: '0 1px 4px rgb(0 21 41 / 30%)',
  backgroundColor: 'white', color: 'var(--theme-color)'
};
...
function AppSider() {
  ...
  return (
    <div style={gcu.siderStyle} className={styles.siderWrap}>
      <Logo />
      <SiderMenus />
      <Button onClick={settings.toggleCollapsed} style={stBtn}>
